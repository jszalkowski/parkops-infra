#--------------------------------------------------------------
# This module creates all resources necessary for Toadexec
#--------------------------------------------------------------

variable "name"               { default = "toadexec" }
variable "region"             { }
variable "vpc_id"             { }
variable "vpc_cidr"           { }
...
resource "aws_security_group" "toadexec" {
  name        = "${var.name}"
  vpc_id      = "${var.vpc_id}"
  description = "Security group for Toadexec"
...
resource "template_file" "user_data" {
  count    = "${var.nodes}"
  template = "${path.module}/toadexec.ps1.tpl"
...
resource "aws_instance" "toadexec" {
  count         = "${var.nodes}"
  ...
  vpc_security_group_ids = ["${aws_security_group.toadexec.id}"]
...
resource "aws_security_group" "elb" {
  name   = "${var.name}-elb"
  vpc_id = "${var.vpc_id}"
  description = "Security group for Toadexec ELB"
...
resource "aws_elb" "toadexec" {
  ...
  instances       = ["${aws_instance.toadexec.*.id}"]
...
resource "aws_route53_record" "toadexec" {
  ...
  name    = "toadexec.${var.sub_domain}"
  type    = "CNAME"
  ttl     = "5"
  records = ["${aws_elb.toadexec.dns_name}"]
}

output "private_ips"  { value = "${join(",", aws_instance.toadexec.*.private_ip)}" }
output "elb_dns"      { value = "${aws_elb.toadexec.dns_name}" }
output "private_fqdn" { value = "${aws_route53_record.toadexec.fqdn}" }
